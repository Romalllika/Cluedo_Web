...
$commonMatchesForReport = (!$isMe)
    ? get_profile_common_matches_for_report($viewerId, (int) $user['id'])
    : [];

$profileHeaderCounts = get_profile_header_counts(
    $viewerId,
    (int) $user['id'],
    $stats,
    $friendsCount,
            ...
            <?php if (!$isMe): ?>
                <section class="panel" id="profile-report">
                    <div class="section-head">
                        ...
                        <small>Жалоба привязывается к общему матчу, чтобы модератор мог проверить логи.</small>
                    </div>

                    <?php if (!$commonMatchesForReport): ?>
                        <p>
                            У вас пока нет общих матчей с этим игроком, поэтому жалобу из профиля отправить нельзя.
                            Во время матча жалоба доступна рядом с игроком в списке участников.
                        </p>
                    <?php else: ?>
                        <form action="profile_report_action.php" method="post" class="profile-report-form">
                                ...
                                <select name="game_id" required>
                                    <?php foreach ($commonMatchesForReport as $match): ?>
                                        <option value="<?= (int) $match['id'] ?>">
                                            #<?= (int) $match['id'] ?>
                                            · <?= h($match['title']) ?>
                                            · <?= h(profile_match_status_label($match['status'])) ?>
                                            · <?= h($match['map_id']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <label>
                                ...
                                <textarea name="comment" rows="5" maxlength="1000"
                                    placeholder="Кратко опиши, что произошло"></textarea>
                            </label>
