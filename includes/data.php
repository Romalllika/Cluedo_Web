...
function characters(): array {
    return [
        ['name' => 'Алекс Громов',       'x' => 4,  'y' => 14, 'color' => '#e53935'],
        ['name' => 'Мария Скарлет',     'x' => 4,  'y' => 9,  'color' => '#d81b60'],
        ['name' => 'Профессор Фиолетов','x' => 10, 'y' => 14, 'color' => '#7e57c2'],
        ['name' => 'Виктор Олив',       'x' => 1,  'y' => 4,  'color' => '#689f38'],
        ['name' => 'Елена Белая',       'x' => 7,  'y' => 4,  'color' => '#eceff1'],
        ['name' => 'София Синяя',       'x' => 13, 'y' => 4,  'color' => '#42a5f5'],
    ];
}
...
function board_paths(int $gid = 0): array {
    ...
    // Верхний коридор под тремя верхними комнатами
    for ($x = 1; $x <= 13; $x++) {
        $add($x, 4);
    }
    ...
    // Нижний проход к Гостиной, Холлу и Кабинету
    for ($x = 4; $x <= 10; $x++) {
        $add($x, 13);
    }

    // Небольшие соединители, чтобы варианты дверей всегда были достижимы
    for ($x = 4; $x <= 10; $x++) {
        $add($x, 5);
        $add($x, 7);
        $add($x, 8);
